...
describe("AdvertisementForm", () => {
  ...
  it("should fill form for preRoll", async () => {
    ...
    await user.pointer({
      target: screen.getByLabelText("Vast Tag"),
      keys: "[MouseLeft]",
    })
    await user.keyboard("https://adserver.com/test")

    expect(screen.getByLabelText("Vast Tag")).toHaveValue(
      "https://adserver.com/test"
    )
  })
  ...
  it("should change displayed form", async () => {
    ...
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Roll Type" }),
      "midRoll"
    )
    await user.type(screen.getByLabelText("Timer"), "30%")
  ...
  it("should update with every entry", async () => {
    ...
    const fillInput = async (label: string, text: string) => {
      await user.pointer({
        target: screen.getByLabelText(label),
        keys: "[MouseLeft]",
      })
    ...
    await fillInput(
      "Fallback Vast Tags (Comma separated)",
      "https://adserver.com/test1,https://adserver.com/test2"
    )
    await fillInput("Video Ad Text", "Ad Text")
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Video Ad Text Position" }),
      "bottom left"
    )
    await user.pointer({
      target: screen.getByLabelText("Video Ad Clickable"),
      keys: "[MouseLeft]",
    })
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Banner Vertical Alignment" }),
      "middle"
    )
    await fillInput("Banner Ad Duration", "20")
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Banner Size" }),
      "728x90"
    )
  ...
  it("should show validate Timer", async () => {
    ...
    await user.pointer({
      target: screen.getByLabelText("Timer"),
      keys: "[MouseLeft]",
    })
    ...
    expect(
      screen.getByText(
        "Field must be a number or a percentage between 1% and 99%"
      )
    ).toBeTruthy()
  ...
  it("should show validate Fallback Vast Tags", async () => {
    ...
    const getTarget = (withPatterError?: boolean) =>
      screen.getByRole("textbox", {
        name: `Fallback Vast Tags (Comma separated)${
          withPatterError ?
            " Should be a string of URLs separated by commas"
          : ""
        }`,
      })
    ...
    expect(
      screen.getByText("Should be a string of URLs separated by commas")
    ).toBeTruthy()
